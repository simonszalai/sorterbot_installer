...
aws ec2 create-key-pair --key-name $1 --query 'KeyMaterial' --output text > ~/.aws/ssh_$1_ec2.pem
chmod 400 ~/.aws/ssh_$1_ec2.pem
...
PROJECT_NAME=$1 cdk deploy SorterBotProdStack --require-approval never

# Retrieve newly created RDS instance host
postgresHost=$(aws rds describe-db-instances --filters "Name=db-instance-id,Values=$1-postgres" --query "DBInstances[*].Endpoint.Address" --output text)
...
PG_CONN="postgresql://postgres:$(< PG_PASS)@${postgresHost}:5432/$1"
...
aws ec2 wait instance-running --filters "Name=tag-value,Values=$1-control-panel"
echo "EC2 instance is running."

# Retrieve newly created EC2 instance host (and remove whitespace)
publicDns=$(aws ec2 describe-instances --filters "Name=tag-value,Values=$1-control-panel" --query "Reservations[*].Instances[*].PublicDnsName" --output text)
publicDns="$(echo -e "${publicDns}" | tr -d '[:space:]')"
echo "EC2 Instance DNS is retrieved."

# Install dependencies on EC2 instance
ssh -o "StrictHostKeyChecking no" -i ~/.aws/ssh_$1_ec2.pem ec2-user@"${publicDns}" "bash -s -- uno" < setup_control_panel.sh
echo "EC2 setup complete."

# Build Docker image than start Docker Compose
ssh -o "StrictHostKeyChecking no" -i ~/.aws/ssh_$1_ec2.pem ec2-user@"${publicDns}" "docker build -t sorterbot_control:latest sorterbot_control; cd sorterbot_control; EXT_PORT=80 DISABLE_AWS=0 MIGRATE=1 docker-compose up"

# Set Django password as environment variable on the EC2 instance
ssh -o "StrictHostKeyChecking no" -i ~/.aws/ssh_$1_ec2.pem ec2-user@"${publicDns}" "export DJANGO_SUPERUSER_PASSWORD=$3"
ssh -o "StrictHostKeyChecking no" -i ~/.aws/ssh_$1_ec2.pem ec2-user@"${publicDns}" "sudo pip3 install -r sorterbot_control/sbc_server/requirements.txt"
ssh -o "StrictHostKeyChecking no" -i ~/.aws/ssh_$1_ec2.pem ec2-user@"${publicDns}" "python3 sorterbot_control/sbc_server/manage.py createsuperuser --username $2 --email blank@email.com"

# Collect static files and host them on S3
ssh -o "StrictHostKeyChecking no" -i ~/.aws/ssh_$1_ec2.pem ec2-user@"${publicDns}" "python3 sorterbot_control/sbc_server/manage.py collectstatic
